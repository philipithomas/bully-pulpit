...
export function newsletterSocialImageSpec(newsletter: Newsletter) {
  ...
  const palette = newsletter === 'tidbits' ? latestTidbitsPalette() : undefined
  ...
  return {
    ...
    background: palette?.paper ?? NEWSLETTER_SOCIAL_BACKGROUNDS[newsletter],
    logoPath: palette ? tidbitsAsset(palette, 'wordmark') : config.logo.src,
    logoWidth: width,
