...
- **Chat (Bell)**: site chat sidebar (`src/components/chat/`, `app/api/chat`) streams via AI SDK + AI Gateway (`anthropic/claude-haiku-4.5` on gateway credits, no BYOK) with tools `searchPosts` (local hybrid search, see Search above) and `fetchPost`, capped at 7 steps with the last step forced to prose (`activeTools: []`). The client renders only text and tool parts, and scrubs leading tool-call JSON from text (`src/lib/chat/scrub-leaked-tool-json.ts`) — gpt-oss-120b used to leak unbound call JSON into replies; keep that guard even though the model changed
